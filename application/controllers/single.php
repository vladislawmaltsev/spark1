...
class Single extends CI_Controller {
	...
	public function photo($img_id)
	{
		...
		if($albumName!='') {
			$image_url = base_url().'public/uploads/member_gallery/'.$userNameRow->username.'/'.$albumName.'/'.$image_row->org_photo;
			$backUrl = base_url().'gallery/photos/'.$userNameRow->username.'/'.$albumName;
		} else {
			$image_url = base_url().'public/uploads/member_gallery/'.$userNameRow->username.'/'.$image_row->org_photo;

...
<div class="siteWraper">
  ...
  <div class="container"> 
    <!--Recent Profiles Start-->
        ...
        <div class="actionBox"> <a href="javascript:window.history.back();" title="Block this Person"><span>Выйти</span></a>
          <div class="clear"></div>
        </div>
        ...
        <div class="albumBox">
          <div class="toheading">
            <h5>Альбомы</h5>
            <span>(<?php echo ($album_row == 0)?'0':count($album_row);?> альбомов)</span>
            <div class="albumLinks"> 
            <?php 
			  if($row->username == $this->session->userdata('username')) {
			  ?>
            <a href="javascript:;" onclick="addAlbumForm('<?php echo $this->session->userdata('username');?>')" class="viewlinks">Добавить новый</a>
            <?php
			  }
			?>
            <a href="" class="viewlinks">Показать все</a> </div>
            <div class="clear"></div>
          </div>
          <?php
						if($album_row):
        ...
        <div class="albumBox">
          <div class="toheading">
            <h5><?php echo ($album_name=='')?'Фото':$album_name;?></h5>
            <span>(<?php echo ($gallery_row == 0)?'0':count($gallery_row);?> фото)</span>
            <div class="albumLinks"> 
            <?php 
			  if($row->username == $this->session->userdata('username')) {
			  ?>
            <a href="javascript:" onclick="addNewPhoto('<?php echo $album_name;?>')" class="viewlinks">Добавить новое</a>
            <?php
			  }
			?>
            <a href="" class="viewlinks">Показать все</a> </div>
